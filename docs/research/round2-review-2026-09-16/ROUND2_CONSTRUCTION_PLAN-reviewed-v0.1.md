...
版本 0.1 · 2026-09-16（北京时间） · **IN_REVIEW：方案已形成，未授权实施本方案或启动新交易日。**
...
以下均为待实施任务；本次已完成的是审查、验证和本方案，不把验证脚本算作生产模块。
...
当前已确定的是第二轮**研究和详细方案**。下一步可以一次性确认T0–T5的离线施工范围和上文建议基线，日常工程不再逐项请求许可。

T6开始前需确定具体交易日与时段、人工来源校验/Gateway登录责任、采用 `OBSERVE_ONLY` 还是已满足条件的 `VALUE_RESEARCH_SHADOW`。正式效果预注册前还需确定δ、ε、h及费用/固定成本口径；它们不阻塞T1的离线价值工具。

本轮交付后保持IN_REVIEW。接受施工不自动接受盈利结论、不自动打开后移事项，也不构成券商委托授权。
